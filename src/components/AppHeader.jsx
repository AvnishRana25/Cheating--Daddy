...
const BookIcon = () => (
    <svg width="24px" height="24px" strokeWidth="1.7" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" color="currentColor">
...
const FlaskIcon = () => (
    <svg width="24px" strokeWidth="1.7" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" color="currentColor">
...
export function AppHeader({
    ...
    const isMac = typeof window !== 'undefined' && window.cheddar?.isMacOS;
            ...
            <div className="header-actions">
                    ...
                    <>
                        ...
                        {statusText && (
                            <span style={{
                                ...
                                fontSize: '12px',
                                fontWeight: '500',
                                color: statusText.includes('Error') ? 'var(--accent-error)' : 
                                       statusText.includes('Ready') ? 'var(--accent-success)' : 'var(--text-secondary)'
                            }}>
                                {statusText}
                            </span>
                        )}
                ...
                {currentView === 'main' && (
                    <>
                        ...
                        <button className="icon-button" onClick={onCustomizeClick} title="Settings">
                            <SettingsIcon />
                        </button>
                        <button className="icon-button" onClick={onHelpClick} title="Help & Shortcuts">
                            <HelpIcon />
                        </button>
                    </>
                )}
                ...
                {currentView === 'assistant' ? (
                    <>
                        <button onClick={onHideToggleClick} className="button" style={{
                            padding: '8px 14px',
                            fontSize: '12px',
                            display: 'flex',
                            alignItems: 'center',
                            ...
                            Hide
                            <span className="key" style={{ pointerEvents: 'none', fontSize: '10px' }}>{isMac ? 'Cmd' : 'Ctrl'}</span>
                            <span className="key" style={{ fontSize: '10px' }}>\</span>
                        </button>
